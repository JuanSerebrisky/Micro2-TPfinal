...
def parse_arguments(argv: List[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--seed", type=int, default=DEFAULT_SEED, help="Semilla global de los generadores aleatorios")
    args, unknown = parser.parse_known_args(argv)
    if unknown:
        print(
            f"Argumentos no reconocidos ignorados: {' '.join(unknown)}",
            file=sys.stderr,
        )
    return args
